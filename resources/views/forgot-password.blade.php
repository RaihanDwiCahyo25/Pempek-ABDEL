...
<head>
    ...
    <title>Lupa Password - Admin Panel Warung Abdel</title>
...
<body class="font-sans antialiased bg-gray-50 text-gray-800 flex h-screen overflow-hidden">
    ...
    <main class="flex-1 flex flex-col relative">
        ...
        <header class="p-8 flex justify-between items-start">
            <div>
                <h2 class="text-2xl font-bold mb-1">Lupa Password</h2>
        ...
        <div class="flex-1 flex justify-center items-center -mt-20">
            <div class="bg-white w-[500px] rounded-xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)] border border-gray-100 overflow-hidden">
                ...
                <div class="p-8">
                    @if (session('status'))
                        <div class="mb-5 rounded-lg bg-green-50 border border-green-100 p-4 text-sm text-green-700">
                            {{ session('status') }}
                        </div>
                    @endif
                    ...
                    <form action="{{ route('password.send-reset-link') }}" method="POST">
                        ...
                        <div class="mb-5">
                            ...
                            <input name="email" type="email" value="{{ old('email') }}" placeholder="Masukkan email Anda" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition">
                        </div>
