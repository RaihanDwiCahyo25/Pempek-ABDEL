...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Create Hero Section</h1>

    <form action="{{ route('admin.hero-sections.store') }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
        @csrf
        ...
        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-bold mb-2">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active') ? 'checked' : '' }} class="mr-2 leading-tight">
                Active
            </label>
            @error('is_active')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
        ...
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.hero-sections.index') }}" class="text-gray-600 hover:text-gray-800">Cancel</a>
        </div>
    </form>
</div>
@endsection